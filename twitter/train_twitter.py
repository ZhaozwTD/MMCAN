...
from utils_twitter import parse_args
from utils_twitter import Optimizer
from utils_twitter import AverageMeter
from utils_twitter import get_logger
from utils_twitter import device
from model import MutualModel
from datasets_twitter import get_data
...
def cal_performance(text_pred, visual_pred, label, theta, beta):
    ...
    total_argmax = torch.argmax(text_pred + visual_pred, dim=1).cpu().numpy()
...
def cal_performance_valid(text_pred, visual_pred, y, label, predict):
    text_pred, visual_pred = F.softmax(text_pred, dim=1), F.softmax(visual_pred, dim=1)
    total_argmax = torch.argmax(text_pred + visual_pred, dim=1).cpu().numpy()
...
def valid_every_epoch(valid_loader, model, logger, args):
    ...
    for (x_texts, x_texts_mask, x_images, x_images_mask, matching_embed), y in valid_loader:
        with torch.no_grad():
            text_outp, visual_outp = model(x_texts, x_texts_mask, x_images, x_images_mask, matching_embed)
            label, predict, _ = cal_performance_valid(text_outp, visual_outp, y, label, predict)
